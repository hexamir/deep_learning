...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 31 16:33:09 2018
...
def generate_triangles(noise=0.0, free_location=False):
    ...
    if free_location:
        ...
        U = (middle, middle+size, middle-size)
        V = (middle+size, middle-size, middle-size)
    imdata1 = generate_a_drawing(figsize, U, V, 0.0)
    imdata2 = generate_a_drawing(figsize, U, V, noise)
...
def generate_dataset_denoising(nb_samples, noise=0.0):
    ...
    for i in range(nb_samples):
        if i % 100 == 0:
            print(i)
        [X1[i], X2[i]] = generate_triangles(noise, True)
    X1 = (X1) / (255)
